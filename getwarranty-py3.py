    ...
    if not asd_db:
        try:
            response = requests.get(
                "https://raw.github.com/stefanschmidt/warranty/master/asdcheck"
            )
            for model, val in [
                model_str.strip().split(":")
                for model_str in response.content.split("\n")
                if model_str.strip()
            ]:
                asd_db[model] = val
        except:
...
def init_model_db():
    ...
    if not model_db:
        try:
            # model_file = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
            # 'model_snippets.pickle'), 'rb')
            model_file = open(
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "model_snippets.json"
                ),
                "r",
            )
...
def online_snippet_lookup(serial):
    if len(serial) == 11:
        ...
        return None
    try:
        prod_xml = requests.get(
            "http://support-sp.apple.com/sp/product",
            params={"cc": snippet, "lang": "en_US"},
        ).content
    ...
    if 10 < len(serial) < 13:
        if len(serial) == 11:
            ...
            if week:
                ...
                year_time += week_dif
            est_date = "" + year_time.strftime("%Y-%m-%d")
        else:
            # New format
...
def offline_estimated_applecare_end_date(details):
    manu_date = details["EST_MANUFACTURE_DATE"]
    ...
    if iOS_device.match(prod_descr):
        ...
        return "" + apple_year_offset(dateutil.parser.parse(manu_date), 2).strftime(
            "%Y-%m-%d"
        )
    else:
        # Mac: Use date of manufacture + 3 years for max AppleCare coverage
        return "" + apple_year_offset(dateutil.parser.parse(manu_date), 3).strftime(
            "%Y-%m-%d"
        )
...
def offline_estimated_warranty_end_date(details):
    manu_date = details["EST_MANUFACTURE_DATE"]
    return "" + apple_year_offset(dateutil.parser.parse(manu_date), 1).strftime(
        "%Y-%m-%d"
    )
...
def warranty_generator(*serials):
    ...
    for serial in serials:
        if (
            not hasattr(serial, "strip")
            and hasattr(serial, "__getitem__")
            ...
            if not prod_descr:
                ...
                try:
                    ...
                    prod_descr = None
            if not prod_descr:
                prod_dict["ERROR_CODE"] = "Unknown model snippet"
            try:
                prod_dict["PROD_DESCR"] = "" + prod_descr
            except:
                prod_dict["PROD_DESCR"] = ""
            # Fill in some details with estimations
            try:
                prod_dict["EST_MANUFACTURE_DATE"] = offline_estimated_manufacture(
                    serial
                )
            except:
                prod_dict["EST_MANUFACTURE_DATE"] = ""
            if prod_dict["EST_MANUFACTURE_DATE"]:
                # Try to estimate when coverages expire
                prod_dict["EST_PURCHASE_DATE"] = "" + prod_dict["EST_MANUFACTURE_DATE"]
                prod_dict[
                    "EST_WARRANTY_END_DATE"
                ] = offline_estimated_warranty_end_date(prod_dict)
                prod_dict[
                    "EST_APPLECARE_END_DATE"
                ] = offline_estimated_applecare_end_date(prod_dict)
                if datetime.datetime.now() > dateutil.parser.parse(
                    prod_dict["EST_APPLECARE_END_DATE"]
                ):
                    prod_dict["EST_WARRANTY_STATUS"] = "EXPIRED"
                elif datetime.datetime.now() > dateutil.parser.parse(
                    prod_dict["EST_WARRANTY_END_DATE"]
                ):
                    prod_dict["EST_WARRANTY_STATUS"] = "APPLECARE"
                else:
                    prod_dict["EST_WARRANTY_STATUS"] = "LIMITED"
            yield prod_dict
...
def warranty(*serials):
    ...
    if (len(serials) == 1) and (len(results) == 1):
        if (
            hasattr(serials[0], "strip")
            and hasattr(serials[0], "__getitem__")
            and not hasattr(serials[0], "__iter__")
        ):
            return results[0]
    return results
...
def main():
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hf:o:ct", ["help", "file=", "output=", "csv", "tsv"]
        )
    ...
    if not format:
        ...
        for result in warranty_dicts:
            ...
            plain_format += "%s: %s\n" % ("PROD_DESCR", result["PROD_DESCR"])
            for key, val in sorted(list(result.items()), key=lambda x: x[0]):
                if key not in ("SERIAL_ID", "PROD_DESCR", "ERROR_CODE"):
                    plain_format += "%s: %s\n" % (key, val)
        ...
        header_row = ["SERIAL_ID", "PROD_DESCR"]
        for key, val in sorted(list(sample_machine.items()), key=lambda x: x[0]):
            if key not in ("SERIAL_ID", "PROD_DESCR", "ERROR_CODE"):
                header_row.append(key)
        header_row.append("ERROR_CODE")
        writer.writerow(header_row)
        for result in warranty_dicts:
            row_data = [result["SERIAL_ID"]]
            row_data.append(result["PROD_DESCR"])
            for key, val in sorted(list(result.items()), key=lambda x: x[0]):
                if key not in ("SERIAL_ID", "PROD_DESCR", "ERROR_CODE"):
                    row_data.append(val)
            row_data.append(result["ERROR_CODE"])
